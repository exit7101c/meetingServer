...
//                fileMap.put("fileKey", commonService.getEncoding(getFileKey));
    ...
    @Transactional(propagation = Propagation.REQUIRED, readOnly = false)
    public HashMap<String, Object> getChiceImsiTap(HashMap<String, Object> param) throws Exception {
        HashMap<String, Object> resultMap = new HashMap<>();
        param.put("userId", param.get("ssUserId"));
        if(param.get("isUpdateYn").toString().equals("Y")){
          exqueryService.update("cronies.app.choice.updateChoiceAroundLatLon", param);
        }

        List<HashMap<String, Object>> aroundList = exqueryService.selectList("cronies.app.choice.callChoiceAroundProcedure", param);
//        resultMap = exqueryService.selectPagingList("cronies.app.choice.getChiceImsiTap", param,
//                Integer.parseInt(param.get("pageNo").toString()),
//                Integer.parseInt(param.get("pageSize").toString()));

        for(int i = 0; aroundList.size() > i; i++){
            aroundList.get(i).put("userKey", commonService.getEncoding(String.valueOf(aroundList.get(i).get("userId"))));
        }
        resultMap.put("result", aroundList);
        return resultMap;
    }

    @Transactional(propagation = Propagation.REQUIRED, readOnly = false)
    public HashMap<String, Object> getChiceImsiTapLocation(HashMap<String, Object> param) throws Exception {
        HashMap<String, Object> resultMap = new HashMap<>();
        resultMap = exqueryService.selectOne("cronies.app.choice.getChiceImsiTapLocation", param);
        return resultMap;
    }
...
//                        }
                ...
                resultMap.put("message", "대상을 24시간 이내 좋아요 하였습니다.");
            }
    ...
    public HashMap<String, Object> setPartnerSkip(HashMap<String, Object> param) throws Exception {
        ...
        if (param.containsKey("partnersCode") && !param.get("partnersCode").toString().isEmpty()) {
            ...
            // 클라이언트에서 item.partnersCode를 넘겨주면 안됨 userKey를 넘겨줘야함. 해당부분 수정하면서 이 부분도 제거하면 됨
            // 바로 위 부분은 프로시저에서 partnersCode를 넘겨주는거로 해결하였으나 스오이프로 스킵 시 partnersCode가 '야간개장'으로 들어가는
            // 이슈 발생 현재 JOB에서 (JOB_SET_YAGAN_UPDATE_SAMPLE) 강제로 업데이트처리하고있음
            param.put("partnersCode", "AD_001");
            HashMap<String, Object> partnersUserInfo = exqueryService.selectOne("cronies.app.partners.partnersUserInfo", param);
            String targetUserId = "";
            if (!MapUtils.isEmpty(partnersUserInfo)) {
                targetUserId = partnersUserInfo.get("userId").toString();
...
//            resultMap.put("message", "에러가 발생했습니다. 초이스 화면을 나간 후 다시 시도해주세요.");
        ...
        if(targetMeLikeMap != null){
            try {
                ...
                if(createdChatroomMap == null){
                    ...
                    resultMap.put("chatroomKey", commonService.getEncoding(String.valueOf(createdChatroomMap.get("chatroomId"))));
    ...
    public HashMap<String, Object> setSuperLikeMatchChatRoom(HashMap<String, Object> param) throws Exception {
        HashMap<String, Object> resultMap = new HashMap<>();
        HashMap<String, Object> pushMap = new HashMap<>();
        ...
        try {
            ...
            if(createdChatroomMap == null){
                ...
                resultMap.put("successYn", "MATCH");
                resultMap.put("effectType", "MATCH");
                resultMap.put("message", "슈퍼라이크로 메세지를 보냈습니다! 메세지 탭으로 이동하시겠습니까?");
            } else {
                resultMap.put("successYn", "MATCH");
                resultMap.put("effectType", "MATCH");
                resultMap.put("message", "매칭 된 상대입니다! 메세지 탭으로 이동하시겠습니까?");
            ...
            resultMap.put("successYn","N");
            resultMap.put("message","슈퍼라이크 중 에러가 발생하였습니다. 문의 부탁드립니다.");
        }
    ...
    public HashMap<String, Object> getUserReceiveLikeCnt(HashMap<String, Object> param) throws Exception {
        HashMap<String, Object> resultMap = new HashMap<>();
        HashMap<String, Object> freeFlipCntMap = new HashMap<>();
        // 받은 좋아요 수
        resultMap = exqueryService.selectOne("cronies.app.choice.getUserReceiveLikeCnt", param);
        // 받은 좋아요 확인하기 사용했는지
        freeFlipCntMap = exqueryService.selectOne("cronies.app.choice.getFreeFlipInfo", param);
        resultMap.put("freeFlipInfo", freeFlipCntMap.get("freeFlipInfo"));
        if(Integer.parseInt(freeFlipCntMap.get("useCnt").toString()) > 0){
            resultMap.put("freeFlipCnt", 0);
        } else {
            resultMap.put("freeFlipCnt", 1);
        }
        return resultMap;
    }
    ...
    public HashMap<String, Object> getFreeFlipIsPossible(HashMap<String, Object> param) throws Exception {
        HashMap<String, Object> resultMap = new HashMap<>();
        HashMap<String, Object> cntMap = new HashMap<>();
        cntMap = exqueryService.selectOne("cronies.app.choice.getFreeFlipIsPossible", param);
        if(Integer.parseInt(cntMap.get("useCnt").toString()) < 1){
            // 사용내역없음(사용가능한상태)
            resultMap.put("successYn", "Y");
        } else {
            // 사용내역있음(사용불가능한상태)
    ...
    public HashMap<String, Object> setFreeFlip(HashMap<String, Object> param) throws Exception {
        HashMap<String, Object> resultMap = new HashMap<>();
        if(exqueryService.insert("cronies.app.choice.insertFreeFlipInfo", param) == 1){
            if(exqueryService.update("cronies.app.choice.updateScLikeFreeFlip", param) == 1){
                resultMap.put("successYn", "Y");
            } else {
                resultMap.put("successYn", "N");
                resultMap.put("message", "받은 좋아요 확인하기 중 오류가 발생하였습니다. 문의 부탁드립니다.");
            }
        } else {
            resultMap.put("successYn", "N");
            resultMap.put("message", "받은 좋아요 확인하기 중 오류가 발생하였습니다. 문의 부탁드립니다.");
