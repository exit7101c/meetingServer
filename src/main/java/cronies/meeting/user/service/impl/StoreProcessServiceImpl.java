...
public class StoreProcessServiceImpl implements StoreProcessService {
    ...
    public HashMap<String, Object> useHeartAttack(HashMap<String, Object> param) throws Exception {
        ...
        HashMap<String, Object> messageSendMap = new HashMap<String, Object>();
        param.put("userId", param.get("ssUserId"));
        ...
        if(blockMap == null) {
            ...
            if(createdChatroomMap == null){
                ...
                if(resultMap.get("successYn").toString().equals("Y")){
                    ...
                    // 매치여부 확인 후 채팅방생성
                    returnMap = choiceService.setSuperLikeMatchChatRoom(param);
                    if(!returnMap.get("successYn").toString().equals("N")){
                        // 채팅방 메세지 전송
                        messageSendMap.put("ssUserId", param.get("ssUserId"));
                        messageSendMap.put("chatroomId", returnMap.get("chatroomId"));
                        messageSendMap.put("message", param.get("message"));
                        messageSendMap.put("type", "chat");
                        messageSendMap.put("fileId", "");
                        messageMainService.sendMessageSingle(messageSendMap);
            ...
            // 차단했을 경우
            resultMap.put("successYn", "N");
            resultMap.put("message", "상대방이 대화를 원치 않아 대화방 생성이 어렵습니다.");
        }
...
//                    resultMap.put("remainingTimeStr", selectMap.get("remainingTimeStr"));
                ...
                resultMap.put("successYn", "N");
                resultMap.put("message", "삭제 된 소모임입니다.");
            }
