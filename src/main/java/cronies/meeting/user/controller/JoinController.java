...
//        format.setTimeZone(TimeZone.getTimeZone("GMT"));
    ...
    @ResponseBody
    @RequestMapping(value = "setDropReset", method = { RequestMethod.GET, RequestMethod.POST })
    public HashMap<String, Object> setDropReset(@RequestParam HashMap<String, Object> param) throws Exception {
        HashMap<String, Object> resultMap = new HashMap<>();

        if (exqueryService.update("cronies.app.join.setDropReset", param) == 1) {
            resultMap.put("successYn","Y");
            resultMap.put("message","계정이 복구되었습니다.");
        } else {
            resultMap.put("successYn","N");
            resultMap.put("message","오류가 발생했습니다. 문의 부탁드립니다.");
        }
        return resultMap;
    }
